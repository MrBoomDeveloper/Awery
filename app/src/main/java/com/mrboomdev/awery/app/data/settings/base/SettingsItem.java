...
public class SettingsItem implements Serializable, ObservableSettingsItem {
	...
	public Object getValue() {
		if(getType() == null) {
			...
			if(getExcludableValue() != null) {
				return getExcludableValue();
			}
		...
		return switch(getType()) {
			case STRING, SELECT, JSON -> getStringValue();
			case BOOLEAN, SCREEN_BOOLEAN -> getBooleanValue();
			case INTEGER, SELECT_INTEGER, COLOR -> getIntegerValue();
			case DATE -> getLongValue();
			case MULTISELECT -> getStringSetValue();
			case SERIALIZABLE -> getSerializable();

			case EXCLUDABLE -> getExcludableValue() == null
					? null : getExcludableValue().name();
	...
	public void onClick(Context context) {
		SettingsActions.run(new Setting.Builder()
				.setKey(getKey())
